...
// "ידע חסר" — sheet נפרד (לא חלק ממצבי הלקוחות) שמציג שאלות שהמאמן AI לא מצא עליהן ידע,
// מקובצות לפי שאלה זהה, מ-30 הימים האחרונים.
async function _fetchKnowledgeGaps() {
    ...
    return { total: (data || []).length, rows };
}

// עדכון תגית המספר על כפתור "ידע חסר" — נקרא בכל פתיחת לוח המנהל
async function _refreshKnowledgeGapsBadge() {
    const badge = document.getElementById('knowledge-gaps-badge');
    if (!badge) return;
    try {
        const { rows } = await _fetchKnowledgeGaps();
        if (rows.length) { badge.textContent = rows.length; badge.style.display = 'inline-block'; }
        else { badge.style.display = 'none'; }
    } catch { badge.style.display = 'none'; }
}

async function openKnowledgeGapsSheet() {
    ...
    try {
        const { total, rows } = await _fetchKnowledgeGaps();
        if (!rows.length) {
            content.innerHTML = '<div class="admin-empty">אין שאלות ללא כיסוי ב-30 הימים האחרונים <span style="display:inline-flex;color:#22c55e;vertical-align:-2px;"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></div>';
            return;
        }
        content.innerHTML = `<div style="font-size:13px;color:var(--text-secondary);margin-bottom:14px;">${total} שאלות ב-30 הימים האחרונים (${rows.length} ייחודיות) — ממוין מהנפוצה ביותר</div>`;
        rows.forEach(r => {
            const row = document.createElement('div');
            row.className = 'coach-overview-card';
            ...
            row.innerHTML =
                `<div style="font-weight:bold;font-size:15px;color:var(--text-primary);">${_esc(r.message)}</div>` +
        ...
        row.innerHTML = `
            <div class="coach-urgent-left">
                ...
                <div class="coach-urgent-text">
                    ...
                    <span class="coach-urgent-reason" style="color:#60a5fa;font-weight:600;">${client.subscription_type === 'bonus' ? '<svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px"><rect x="3" y="9" width="18" height="12" rx="1.5"/><path d="M3 13h18"/><path d="M12 9v12"/><path d="M12 9c-1.5-4-6-4-6-1.5S9 9 12 9z"/><path d="M12 9c1.5-4 6-4 6-1.5S15 9 12 9z"/></svg> מנוי בונוס ליווי' : client.subscription_type === 'paid' ? '<svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px"><rect x="2.5" y="5" width="19" height="14" rx="2.2"/><path d="M2.5 10h19"/><path d="M8 15l1.5 1.5L13 13"/></svg> מנוי בתשלום' : '<svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px"><rect x="2.5" y="5" width="19" height="14" rx="2.2"/><path d="M2.5 10h19"/><path d="M8 15l1.5 1.5L13 13"/></svg> מנוי פעיל'}</span>
                    ${client.subscription_type === 'paid' ? `<span class="coach-urgent-reason" style="color:${client.auto_billing ? '#4ade80' : '#facc15'};font-weight:600;">${client.auto_billing ? '<svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px"><path d="M4 5h16a1 1 0 0 1 1 1v9a1 1 0 0 1-1 1H9l-4 3.5V16H4a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1z"/><path d="M8 10.5h.01"/><path d="M12 10.5h.01"/><path d="M16 10.5h.01"/></svg> אוטומטי אמיתי' : '<svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px"><path d="M8 12V5a1.5 1.5 0 0 1 3 0v5"/><path d="M11 10V4a1.5 1.5 0 0 1 3 0v6"/><path d="M14 10V5a1.5 1.5 0 0 1 3 0v8"/><path d="M17 13v-2a1.5 1.5 0 0 1 3 0v5a6 6 0 0 1-6 6h-2a6 6 0 0 1-5-3l-2.5-4.5a1.5 1.5 0 0 1 2.5-1.5L8 15"/></svg> מסומן ידנית'}</span>` : ''}
                    ${(() => {
...
function _renderUrgentMode(list) {
    ...
    if (!list.children.length) {
        list.innerHTML = '<div class="admin-empty"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round" style="vertical-align:-3px"><path d="M12 3l1.5 6L20 10.5 14 12l-2 6-2-6-6-1.5L10.5 9z"/></svg> כל הלקוחות במצב תקין!</div>';
    }
}
