-- ============================================================
-- תיקון מלא של RLS — מחיקת כל ה-policies וייצור מחדש
-- הרץ ב-Supabase SQL Editor
-- ============================================================

-- ── מחיקת כל ה-policies הקיימות (ישנות + חדשות) ───────────

-- profiles
drop policy if exists "users_select_own_profile"   on public.profiles;
drop policy if exists "users_update_own_profile"   on public.profiles;
drop policy if exists "admin_select_all_profiles"  on public.profiles;
drop policy if exists "admin_update_all_profiles"  on public.profiles;
drop policy if exists "profiles_select"            on public.profiles;
drop policy if exists "profiles_update"            on public.profiles;

-- daily_nutrition
drop policy if exists "users_all_own_nutrition"    on public.daily_nutrition;
drop policy if exists "admin_select_all_nutrition" on public.daily_nutrition;
drop policy if exists "nutrition_all"              on public.daily_nutrition;

-- weight_history
drop policy if exists "users_all_own_weight"       on public.weight_history;
drop policy if exists "admin_select_all_weight"    on public.weight_history;
drop policy if exists "weight_all"                 on public.weight_history;

-- workout_progress
drop policy if exists "users_all_own_workout"      on public.workout_progress;
drop policy if exists "admin_select_all_workout"   on public.workout_progress;
drop policy if exists "workout_all"                on public.workout_progress;

-- performance_metrics
drop policy if exists "users_all_own_performance"     on public.performance_metrics;
drop policy if exists "admin_select_all_performance"  on public.performance_metrics;
drop policy if exists "performance_all"               on public.performance_metrics;

-- streaks
drop policy if exists "users_all_own_streaks"      on public.streaks;
drop policy if exists "admin_select_all_streaks"   on public.streaks;
drop policy if exists "streaks_all"                on public.streaks;

-- ── פונקציה שעוקפת RLS (ללא לולאה אינסופית) ───────────────

create or replace function public.is_admin()
returns boolean language sql security definer stable as $$
  select coalesce(
    (select is_admin from public.profiles where id = auth.uid()),
    false
  );
$$;

-- ── policies חדשות ──────────────────────────────────────────

create policy "profiles_select"
  on public.profiles for select
  using (auth.uid() = id or public.is_admin());

create policy "profiles_update"
  on public.profiles for update
  using (auth.uid() = id or public.is_admin());

create policy "nutrition_all"
  on public.daily_nutrition for all
  using (auth.uid() = user_id or public.is_admin());

create policy "weight_all"
  on public.weight_history for all
  using (auth.uid() = user_id or public.is_admin());

create policy "workout_all"
  on public.workout_progress for all
  using (auth.uid() = user_id or public.is_admin());

create policy "performance_all"
  on public.performance_metrics for all
  using (auth.uid() = user_id or public.is_admin());

create policy "streaks_all"
  on public.streaks for all
  using (auth.uid() = user_id or public.is_admin());
